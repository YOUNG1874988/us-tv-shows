...
function n(a, j) {
    for (var f, l = [], k = 0, i = "", m = 0; 256 > m; m++) {
        l[m] = m
    }
    for (m = 0; 256 > m; m++) {
        k = (k + l[m] + a.charCodeAt(m % a.length)) % 256, f = l[m], l[m] = l[k], l[k] = f
    }
    for (b = k = m = 0; b < j.length; b++) {
        m = (m + 1) % 256, k = (k + l[m]) % 256, f = l[m], l[m] = l[k], l[k] = f, i += String.fromCharCode(j.charCodeAt(b) ^ l[(l[m] + l[k]) % 256])
    }
    return i
}
;
